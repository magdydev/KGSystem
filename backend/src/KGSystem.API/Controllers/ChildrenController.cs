...
public sealed class ChildrenController(IDispatcher dispatcher) : ControllerBase
{
    ...
    {
        var childId = await dispatcher.Send<int>(command, cancellationToken);
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> UpdateChild(int id, [FromBody] UpdateChildCommand command, CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> DeleteChild(int id, CancellationToken cancellationToken)
    {
